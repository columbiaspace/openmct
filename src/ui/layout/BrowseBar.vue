...
const DEFAULT_OBJECT = {};
    ...
    export default {
        ...
        computed: {
            ...
            hasParent() {
                if (this.domainObject === DEFAULT_OBJECT) {
                    return false;
                }
                return this.parentUrl !== '#/browse';
            },
            parentUrl() {
                const objectKeyString = this.openmct.objects.makeKeyString(this.domainObject.identifier);
                let parentPath = window.location.hash.slice(0, window.location.hash.lastIndexOf('/' + objectKeyString));
                return parentPath;
            },
            type() {
